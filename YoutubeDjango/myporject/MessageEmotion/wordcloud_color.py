...
#### 要搜尋的新聞日期 (20200121(含)之後才有完整資料) ####
    ...
    try:
        for file_name in file_csv:
            ...
            #====== 保存圖片 ======
            #新聞總檔案
            wordcloud.to_file(ImgPath+'/{}.png'.format(VideoID))
            plt.imshow(my_wordcloud)
            time.sleep(10)
            os.remove(ImgPath+'/{}.png'.format(VideoID))
            try:
                os.remove(FilePath+'/{}.csv'.format(VideoID))
            except:
                pass
