...
while True:  # 每秒檢查是否有csv
    ...
    try:
        if len(file_csv) != 0:
            for Csv_name in file_csv:
                TxtName=Csv_name[:-4]
                Article=pd.read_csv(pwd+r"/{}".format(Csv_name))
                Article_list=Article.values.tolist()
                for i in Article_list:
                    Article=GPT2Article(i[0])
                print(Article)
                TxtOpen=open(pwd+r"/{}.txt".format(TxtName),"w",encoding="utf-8-sig")
                TxtOpen.write(Article)
                TxtOpen.close()
                time.sleep(5)
                os.remove(pwd+r"/{}.txt".format(TxtName))
                os.remove(pwd+r"/{}.csv".format(TxtName))
    except:
        pass
